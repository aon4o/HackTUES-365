...
class Tutorial < Gosu::Window
  ...
  def update
    if (Gosu.distance(@opt_a.x+10, @opt_a.y+10, mouse_x, mouse_y) < 20 && button_down?(Gosu::MsLeft))
      puts("you cklicked opt a")
    end
    if (Gosu.distance(@opt_b.x+10, @opt_b.y+10, mouse_x, mouse_y) < 20 && button_down?(Gosu::MsLeft))
      puts("you cklicked opt b")
    end
    if (Gosu.distance(@opt_c.x+10, @opt_c.y+10, mouse_x, mouse_y) < 20 && button_down?(Gosu::MsLeft))
